...
def get_trivia_data():
    ...
    try:
        ...
        return [{"question": "Who developed Delta Force: Hawk Ops?", "options": ["Tencent (TiMi)", "Activision", "EA", "Ubisoft"], "answer": 0}]
...
async def cmd_trivia(event: types.Message | types.CallbackQuery, user_service: UserService, group_service: GroupService):
    ...
    if message.chat.type != "private":
        ...
        if group_info and not group_info.settings.get("trivia_enabled", True):
            if is_callback: await event.answer("❌ Simulations disabled for this sector.", show_alert=True)
            else: await message.answer("❌ Simulations disabled by Sector Command.")
            return
    ...
    if message.chat.type == "private":
        ...
        builder.button(text="👤 PROFILE (DM)", url=f"https://t.me/{bot_user.username}?start=profile")
    builder.adjust(2)
    
    text = get_header("TACTICAL SIMULATION", "🧠")
    text += f"<b>QUESTION:</b>\n{q['question']}\n\n"
    text += "<i>Authorized operators only. Fastest response earns credits.</i>"
    text += "\n" + get_footer("Sim-Sector v1.0")
...
async def process_trivia(callback: types.CallbackQuery, user_service: UserService, group_service: GroupService):
    ...
    if len(parts) == 2 and parts[1] == "done":
        await callback.answer("Simulation finalized.", show_alert=True)
        return
        
    q_idx, chosen, correct, start_time_val = int(parts[1]), int(parts[2]), int(parts[3]), int(parts[4])
    delta = time.time() - start_time_val
    questions = get_trivia_data()
    q = questions[q_idx]
    ...
    if chosen == correct:
        ...
        text = get_header("SIMULATION: SUCCESS", "✅")
        text += f"<b>Q:</b> {q['question']}\n"
        text += format_field("RESULT", "IDENTIFIED", "🎯")
        text += format_field("OPERATOR", user_name, "👤")
        text += format_field("TIME", f"{delta:.1f}s", "⏱️")
        text += format_field("REWARD", "+100 Credits", "💰")
        
        if callback.message.chat.type != "private":
            await group_service.track_member(callback.message.chat.id, callback.from_user.id)
            
        builder = InlineKeyboardBuilder()
        builder.button(text="🧠 RE-ENGAGE", callback_data="main_trivia")
        builder.button(text="🏠 HUB MENU", callback_data="main_menu")
        builder.adjust(1)
        
        await callback.message.edit_text(text, reply_markup=builder.as_markup())
        await callback.answer("Credits secured.")
    else:
        text = get_header("SIMULATION: FAILED", "❌")
        text += f"<b>Q:</b> {q['question']}\n"
        text += format_field("RESULT", "MISIDENTIFIED", "⚠️")
        text += format_field("KEY", q['options'][correct], "🔑")
        
        builder = InlineKeyboardBuilder()
        builder.button(text="🧠 RETRY", callback_data="main_trivia")
        builder.button(text="🏠 HUB MENU", callback_data="main_menu")
        builder.adjust(1)
        
        await callback.message.edit_text(text, reply_markup=builder.as_markup())
        await callback.answer("Accuracy: 0%", show_alert=True)
